...
void InvertedIndex::addDocument(Document doc)
{
    ...
    {
        throw std::invalid_argument("Document with id=" + std::to_string(doc.getId()) + " already exists");
    }
...
std::size_t InvertedIndex::wordCount(Document::Id docId, const std::string& word) const
{
    const std::string normalized = DocumentBuilder::normalize(word);
...
std::optional<std::reference_wrapper<const Document>> InvertedIndex::getDocument(Document::Id id) const
{
    auto it = documents_.find(id);
    if (it == documents_.end())
        return std::nullopt;
