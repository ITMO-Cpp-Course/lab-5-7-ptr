...
Document DocumentBuilder::build(std::string name, std::string content)
{
    ...
    return Document(nextId_++, std::move(name), std::move(content), std::move(tokens));
}
...
std::string DocumentBuilder::normalize(std::string word)
{
    ...
    auto first =
        std::find_if(word.begin(), word.end(), [](char c) { return std::isalpha(static_cast<unsigned char>(c)) != 0; });
    word.erase(word.begin(), first);

    if (word.empty())
        return word;

    auto last = std::find_if(word.rbegin(), word.rend(),
